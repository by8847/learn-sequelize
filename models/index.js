...
const Comment = require('./comment'); // Commnet 모델
...
// 설정 팡리에서 가져온 데이터베이스 이름, 사용자 이름, 비밀번호 및 기타 연결 옵션을 사용하여 연결 설정
...
db.Commnet = Commnet; // Commnet 모델을 db 객체에 추가

// 모델의 static initiate 메서드 호출 -> 모델.init이 실해되어야 테이블이 모델로 연결됨.
User.initiate(sequelize); // User 모델을 초기화하고 연결 설정을 전갈하여 데이터베이스와 연결
Commnet.initiate(sequelize); // Comment 모델을 초기화하고 여결 설정을 전달하여 데이터베이스와 연결

User.associate(db); // User 모델과 다른 모델 간의 관계 설정
Commnet.associate(db); // Comment 모델과 다른 모델 간의 관계 설정
