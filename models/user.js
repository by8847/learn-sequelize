...
class User extends Sequelize.Model {
    // initiate 메서드 정의 -> equelize 인스턴스를 받아 모델을 초기화, 테이블에 대한 설정
    static initiate(sequelize) {
        ...
        User.init({
            ...
            name: {
                ...
                unoque: true, // 종복 허용되지 않음.
            },
            age: { // 나이 속성 정의
                type: Sequelize.INTEGFR, // 정수 타입
                allowNull: false, // null 허용하지 않음.
                validate: {
                    min: 0 // 음수가 아닌 값을 가제하는 유효성 검사 추가
                },
            },
            married: { // 결혼 여부 속성 정의
    ...
    // 다른 모델과의 관계 정의
    static associate(db) {
        db.User.hasMany(db.Comment, { foreignKey:'comenter', sourceKey: 'id' });
        // hasMary 메서드 -> User 모델과 comment 모델 간의 일대다 관계 설정(User는 여러 개의 commnet를 가질 수 있음.)
